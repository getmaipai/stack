...
export const StackEventData = {
  ...
  "job.progress": z.object({ job: z.string(), kind: z.string(), state: z.enum(["queued", "running", "done", "failed", "cancelled"]), percent: z.number().int().min(0).max(100), completed_bytes: z.number().int().min(0), total_bytes: z.number().int().min(0), status: z.string() }).passthrough(),

...
import { fetchLibrary, getLibraryPage, libraryLocation, listLibrary, searchLibrary } from "@/lib/library";
import { showroom, showroomLibrary } from "@/showroom/fixture";

const item = z.object({ id: z.string(), kind: z.enum(["model", "engine"]), title: z.string(), source: z.string(), licence: z.string(), fetchedAt: z.string(), revision: z.string(), size: z.number().int(), location: z.string() });
...
libraryRoutes.openapi(listRoute, (c) => c.json({ library: showroom() ? showroomLibrary : listLibrary().map((entry) => ({ ...entry, location: libraryLocation(entry.id) })) } as never, 200));

...
const BudgetSchema = z.object({ totalMemoryBytes: z.number().int(), capBytes: z.number().int(), freeMemoryBytes: z.number().int(), availablePercent: z.number(), pressure: z.enum(["normal", "warn", "critical"]), loaded: z.array(z.object({ id: z.string(), kind: z.enum(["resident", "jit", "generator"]), peakBytes: z.number().int(), measured: z.boolean(), lastUsedAt: z.string(), idleTtlSeconds: z.number().int(), pinned: z.boolean(), pid: z.number().int().nullable() })), queue: z.array(z.object({ id: z.string(), position: z.number().int(), kind: z.enum(["resident", "jit", "generator"]) })) });
...
hardwareRoutes.openapi(budgetRoute, (c) => c.json(getGovernorStatus(), 200));

...
detectedRoutes.openapi(scanRoute, async (c) => { const rows = showroom() ? showroomRows() : await detectAll(); return c.json({ detected: rows, found: showroom() ? { tools: 1, modelFiles: 3 } : scanCounts(rows), scannedAt: showroom() ? new Date().toISOString() : lastScan() } as never, 200); });

...
import { adoptDetected, detectAll, forgetDetected, lastScan, listDetected, scanCounts, type DetectedRecord } from "@/lib/detect";
...
const scanResponse = z.object({ detected: z.array(DetectedSchema), found: z.object({ tools: z.number(), modelFiles: z.number() }), scannedAt: z.string().nullable() });
...
detectedRoutes.openapi(listRoute, (c) => { const includeForgotten = c.req.valid("query").all === true; const rows = (showroom() ? showroomRows() : listDetected(includeForgotten)).filter((item) => includeForgotten || !item.forgotten); return c.json({ detected: rows, found: showroom() ? { tools: 1, modelFiles: 3 } : scanCounts(rows), scannedAt: showroom() ? new Date().toISOString() : lastScan() } as never, 200); });
detectedRoutes.openapi(scanRoute, (c) => { if (!showroom()) void detectAll(); const rows = showroom() ? showroomRows() : listDetected(); return c.json({ detected: rows, found: showroom() ? { tools: 1, modelFiles: 3 } : scanCounts(rows), scannedAt: showroom() ? new Date().toISOString() : lastScan() } as never, 200); });

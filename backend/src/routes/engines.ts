...
const settingSchema = z.object({
  ...
  range: z.object({ min: z.number().optional(), max: z.number().optional() }).optional(), inEffect: z.union([z.string(), z.number(), z.boolean(), z.array(z.string())]), pending: z.union([z.string(), z.number(), z.boolean(), z.array(z.string())]).nullable(),
});
...
const controlRoute = (action: "start" | "stop" | "restart" | "probe") => createRoute({ method: "post", path: `/{name}/${action}`, tags: ["Engines"], summary: `${action} engine`, middleware: [requireOperator] as const, request: { params: nameParam }, responses: { 200: { content: { "application/json": { schema: z.object({ ok: z.literal(true), healthy: z.boolean().optional() }) } }, description: "Engine control completed." }, 400: { content: { "application/json": { schema: ErrorSchema } }, description: "Engine control failed." }, 404: { content: { "application/json": { schema: ErrorSchema } }, description: "Unknown engine." }, 409: { content: { "application/json": { schema: ErrorSchema } }, description: "The process is managed outside the Stack." } } });
for (const action of ["start", "stop", "restart", "probe"] as const) enginesRoutes.openapi(controlRoute(action), async (c) => {
  const { name } = c.req.valid("param");
  if (action !== "probe") {
    const known = ENGINE_BINARIES.some((pin) => pin.id === name || pin.id.startsWith(`${name}-b`));
    if (!known) return c.json({ error: "Unknown engine." }, 404);

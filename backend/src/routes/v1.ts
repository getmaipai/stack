...
const transcriptionsRoute = createRoute({ method: "post", path: "/audio/transcriptions", tags: ["Roles"], summary: "Speech to text", request: { body: { content: { "application/json": { schema: TranscriptionRequestSchema } } } }, responses: inferenceResponses });
...
v1Routes.openapi(transcriptionsRoute, (c) => reply(c, "transcription", c.req.valid("json")) as never);

...
const CheckRunSchema = z.object({ at: z.string(), ok: z.boolean(), results: z.array(RoleResultSchema), fitTogether: z.object({ ok: z.boolean(), reason: z.string().nullable() }), reason: z.string().nullable(), generation: z.number().int() });
const CheckRunViewSchema = CheckRunSchema.extend({ stale: z.boolean(), staleReason: z.string().nullable() });
...
const latestRoute = createRoute({ method: "get", path: "/latest", tags: ["Readiness"], summary: "The last readiness run", responses: { 200: { content: { "application/json": { schema: z.object({ latest: CheckRunViewSchema.nullable(), running: z.object({ startedAt: z.string() }).nullable() }) } }, description: "The last recorded run (stale once a pin, model, engine or setting changed after it) and whether one is in progress." } } });

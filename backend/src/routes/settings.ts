...
import { getMaintenanceScheduler } from "@/lib/maintenance";
...
const maintenanceRoute = createRoute({ method: "post", path: "/maintenance/run", tags: ["Settings"], summary: "Run maintenance now", middleware: [requireOperator] as const, responses: { 200: { content: { "application/json": { schema: z.object({ state: z.string(), nextRunAt: z.string(), ran: z.array(z.string()), reason: z.string().optional() }) } }, description: "Maintenance result." } } });

export const settingsRoutes = apiRouter();
settingsRoutes.openapi(readRoute, (c) => c.json({ settings: readStackConfig() }, 200));
settingsRoutes.openapi(updateRoute, (c) => { try { return c.json({ settings: updateStackConfig(c.req.valid("json")) }, 200); } catch (error) { const message = error instanceof Error ? error.message : "Invalid Stack setting"; return c.json({ error: message }, 400); } });
...
settingsRoutes.openapi(maintenanceRoute, async (c) => c.json(await getMaintenanceScheduler().run(true), 200));

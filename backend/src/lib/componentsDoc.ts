...
function engineTag(pin: EngineBinaryPin): string { return pin.id.match(/-(b\d+)/)?.[1] ?? "legacy"; }
...
export function renderComponentsDoc(catalog: ComponentsCatalog): string {
  ...
  for (const role of catalog.roles) {
    ...
    for (const pin of pins) lines.push(`| \`${pin.id}\` | ${engineTag(pin)} | ${pin.platform} ${pin.arch}${pin.requiresNvidia ? " (NVIDIA)" : ""} | ${pin.verified ? "pinned, verified" : "pinned, not yet verified"} |`);

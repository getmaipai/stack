...
export function renderComponentsDoc(catalog: ComponentsCatalog): string {
  ...
  for (const role of catalog.roles) {
    ...
    for (const pin of pins) lines.push(`| \`${pin.id}\` | ${pin.tag} | ${pin.platform} ${pin.arch}${pin.requiresNvidia ? " (NVIDIA)" : ""} | ${pin.verified ? "pinned, verified" : "pinned, not yet verified"}${pin.tool === "uv" ? ` (the tool that builds the ${role} environment)` : catalog.managed.some((runtime) => runtime.name === pin.name) ? " (the engine's source; its environment is the row below)" : pin.name === "mlx-serve" ? (role === "embed" ? " (the second chat engine; the embed role stays on llama-server)" : " (the second chat engine, chosen by `stack.engines.chat.engine`)") : ""} |`);
    const modelRole = shares ?? role;
    for (const runtime of catalog.managed.filter((candidate) => candidate.roles.includes(modelRole))) lines.push(`| \`${runtime.name}\` | ${runtime.version} | ${runtime.platforms} | an environment the Stack assembles through the pinned uv from a hashed requirements file (backend/src/speech/ or backend/src/generators/); updates with the Stack's release |`);

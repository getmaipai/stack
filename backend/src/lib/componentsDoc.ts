...
export function renderComponentsDoc(catalog: ComponentsCatalog): string {
  ...
  for (const role of catalog.roles) {
    ...
    for (const pin of pins) lines.push(`| \`${pin.id}\` | ${pin.tag} | ${pin.platform} ${pin.arch}${pin.requiresNvidia ? " (NVIDIA)" : ""} | ${pin.verified ? "pinned, verified" : "pinned, not yet verified"}${pin.tool === "uv" ? ` (the tool that builds the ${role} environment)` : pin.tool && pin.tool !== "llama-server" ? " (the engine's source; its environment is the row below)" : ""} |`);

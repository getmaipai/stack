...
- [ ] **RT-01 (L, after the UI block, owner's call 2026-09-19/20): the four managed runtimes.** The daemon installs, runs and watches llama.cpp (today), Ollama, ComfyUI (STACK-13) and oMLX as first-class runtimes; one design note per runtime in `dev.md` before code (engine kind in the catalog, adoption of a detected install, the wire it serves, its roles, its health row). Program note section 6.
- [ ] **BENCH-01 (M, with RT-01): the runtime bench.** Run the same model on llama.cpp, Ollama and oMLX and show tokens/s, first-token time, load time and measured memory side by side, on the Tester (or a Benchmarks tab of Monitoring) and as a saved series; reuses the speed test's measurement and `lib/series.ts` speed results; engine build, model file and sanitized hardware recorded with every run. Exit: a bench row per runtime for one model, captured.
- [x] **UI-05 (S): the taxonomy declaration.** `frontend/src/lib/taxonomy.ts`: four groups, 17 destinations, eight categories with their subtypes, paths, subtitles, icons and hues, from spec section 2 and 4. A test asserts the lists match the spec word for word. Consumed by the rail, header, footer links, search, tiles and browsers; no second list anywhere (a grep test).
- [x] **UI-01 (M): the token set.** `kit/tokens.css` becomes the spec's navy palette (13 values, section 1), the radii, the shadow, the focus ring, the type scale on the system face, the category hues, and the responsive tiers 720/960/1280; a derived light palette; a contrast test over every text-on-surface pair (AA). `tokens.test.ts` asserts the dark root values.
- [x] **UI-02 (M): the shell.** Rail 252/72 px with the double-chevron toggle persisted per install, groups and labels from UI-05, collapsed icon rail with tooltips, drawer under 720 px; the 64 px fixed header (destination icon, title and subtitle from the taxonomy; the search field; appearance; notifications; the selector slot); the 40 px fixed footer (version, three linked counts, aggregate health); the content region scrolls alone. The run pill, profile menu, phone tab bar and `SectionFrame` description lines go (program decisions 2, 8, 9). Captures at 1440, 1100, 800 and 390. (verified at 4b765bd, header/footer at 252a69f)
- [x] **UI-03 (S): the system pulse.** Five signals in order with dots, the two badges, tooltips, click targets to Alerts and Monitoring, the unavailable state; network from UI-06 (muted until it lands). (verified at 09aeb00, wired into the rail alongside UI-04)
- [x] **UI-04 (S): the machine/stack selector and Lock MaiPai.** This computer's name, OS and health; Lock (logout, the card reads "Locked"), Settings, Help; the Switch stack section only when a remote exists (UI-22). Sign out disappears everywhere. (verified at 09aeb00)
- [x] **UI-06 (S): `GET /stack/v1/network`.** Interface, link speed, gateway latency, nulls when not measurable, cached; a privacy row (LAN gateway only). (verified at ddc840d)
- [x] **UI-07 (M): resource series.** CPU, memory, per-GPU and per-drive samples recorded on a schedule into the measurements store; `GET /stack/v1/series/resources?range=hour|day|week|month`, bucketed, nulls kept; retention; month added to the range type. (verified at 1f83195; the CPU reading is the chat engine's own process CPU, not host-wide, and the memory reader's degraded-fallback can persist a fabricated reading into history: both filed as issues #2 and #3, not fixed here)
- [x] **UI-08 (M): the components aggregate.** `GET /stack/v1/components` (category, subtype, name, identifier, version or size, status, runtime, resources, uptime, last used, notes) across the eight categories from the real stores per program decision 1, with `category` and `status` filters and a summary (installed, running, clients connected, updates); `GET /stack/v1/activity` (durable events) and `GET /stack/v1/notifications` with a read state. (verified at 1ad3616; activity is a filtered read of the existing notifications log, not a new table; closes #2, the resource sampler's CPU reading was the chat engine's own process, now host-wide)
...
- [x] **UI-12 (M): Models on the browser.** Columns, modes, facets, pane tabs and the action rail per the spec's Models section; scroll hierarchy per "Models page scrolling and pinned regions".

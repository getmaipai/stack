...
test("all five signals render in order with an accessible name each", async () => {
  stub(okResponses);
  render(<MemoryRouter><SystemPulse /></MemoryRouter>);
...
test("each signal links to its target", async () => {
  ...
  expect(screen.getByLabelText("Memory").getAttribute("href")).toBe("/monitoring#memory");
  expect(screen.getByLabelText("Storage").getAttribute("href")).toBe("/monitoring#storage");
  expect(screen.getByLabelText("GPU").getAttribute("href")).toBe("/monitoring#gpu");
  expect(screen.getByLabelText("Network").getAttribute("href")).toBe("/monitoring#network");
});

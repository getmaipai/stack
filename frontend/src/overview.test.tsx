...
test("Overview requests the selected series range and renders scripted widgets", async () => {
  ...
  globalThis.fetch = mock((input: RequestInfo | URL) => {
    ...
    if (path.endsWith("/roles")) return Promise.resolve(Response.json({ roles: [{ id: "chat", state: { state: "ready", since: new Date().toISOString(), checkedAt: new Date().toISOString() }, description: "Chat", model: null }] }));

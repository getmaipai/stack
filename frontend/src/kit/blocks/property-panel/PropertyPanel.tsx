...
function PanelBody({ kind, item, status, actions, primaryActions, facts = [], usedBy = [], tabs, onClose }: Omit<PropertyPanelProps, "open">) {
  ...
  return <TooltipProvider><div className="flex h-full flex-col" data-testid="property-panel"><div className="border-b p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{kind}</p><h2 className="mt-1 text-xl font-semibold">{panelTitle(item)}</h2></div><div className="flex items-center gap-1"><Badge variant={status.toLowerCase().includes("offline") ? "destructive" : "secondary"}>{status}</Badge><Button size="icon" variant="ghost" aria-label="Close panel" onClick={onClose}>×</Button></div></div><div className="mt-4 flex flex-wrap gap-1">{actions.map((action) => <Tooltip key={action.label}><TooltipTrigger asChild><Button size="icon" variant="ghost" className="size-9" aria-label={action.label} disabled={action.disabled} onClick={() => runAction(action)}><ActionIcon name={action.icon} /></Button></TooltipTrigger><TooltipContent>{action.label}</TooltipContent></Tooltip>)}</div>{facts.length > 0 && <div className="mt-4 grid gap-2 border-y py-3 text-sm sm:grid-cols-3">{facts.slice(0, 3).map((fact) => <div key={fact.label}><p className="text-xs text-muted-foreground">{fact.label}</p><p className="mt-1 font-medium tabular-nums">{fact.value}</p></div>)}</div>}{primaryActions && <div className="mt-4 grid grid-cols-2 gap-2">{primaryActions.map((action) => <Button key={action.label} variant="outline" disabled={action.disabled} onClick={() => runAction(action)}>{action.label}</Button>)}</div>}{confirm && <Card className="mt-4 border-destructive"><CardContent className="space-y-3 p-3"><p className="text-sm">{confirm.confirmLabel ?? `${confirm.label} ${panelTitle(item)}?`}</p><div className="flex justify-end gap-2"><Button size="sm" variant="outline" onClick={() => setConfirm(null)}>Cancel</Button><Button size="sm" variant="destructive" onClick={async () => { await confirm.onClick(); setConfirm(null); }}>Confirm</Button></div></CardContent></Card>}</div><Tabs defaultValue="overview" className="min-h-0 flex-1 overflow-y-auto"><TabsList variant="line" className="w-full justify-start rounded-none border-b px-4 py-2">{tabEntries.map((tab) => { const Icon = getIcon(tab.icon); return <Tooltip key={tab.id}><TooltipTrigger asChild><TabsTrigger value={tab.id} aria-label={tab.label}><Icon className="size-4" /></TabsTrigger></TooltipTrigger><TooltipContent>{tab.label}</TooltipContent></Tooltip>; })}</TabsList>{tabEntries.map((tab) => <TabsContent key={tab.id} value={tab.id} className="space-y-5 p-5">{tab.content}</TabsContent>)}</Tabs>{usedBy.length > 0 && <div className="border-t p-5"><UsedBy items={usedBy} /></div>}</div></TooltipProvider>;
}

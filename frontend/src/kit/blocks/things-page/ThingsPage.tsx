...
export function ThingsPage({ filter, table, panel }: ThingsPageProps) {
  ...
  return <div className={panel ? "pr-0 lg:pr-[29rem]" : undefined} data-testid="things-page">{phone ? <ChipRow chips={phoneChips} active="All" /> : <div className="mb-3 flex items-center justify-between gap-3 lg:hidden"><Button type="button" variant="outline" onClick={() => setMobileFiltersOpen(true)}><Filter className="size-4" />Filter</Button></div>}<div className="flex min-w-0 items-start gap-5">{wide && !phone && <FilterColumn {...filter} collapsed={filterCollapsed} onCollapsedChange={setFilterCollapsed} />}<div className="min-w-0 flex-1">{table}</div></div>{panel && !phone ? panel : null}<Sheet open={mobileFiltersOpen} onOpenChange={setMobileFiltersOpen}><SheetContent side="left" className="w-80 max-w-[calc(100vw-1rem)] p-5"><SheetHeader className="sr-only"><SheetTitle>Filters</SheetTitle><SheetDescription>Filter this table.</SheetDescription></SheetHeader><FilterColumn {...filter} collapsed={false} onCollapsedChange={() => setMobileFiltersOpen(false)} /></SheetContent></Sheet></div>;
}

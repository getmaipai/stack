...
import type { BudgetResponse, HardwareInfo, HealthItem, RepairRecord, RoleRecord } from "@/lib/api";
...
const iconMap = { Overview: "LayoutDashboard", Engines: "Cpu", Models: "Box", Clients: "KeyRound", Tester: "Bot", Monitoring: "Gauge", Settings: "Settings", Logs: "FileText", Alerts: "Bell" } as const;
const DOT = { good: "bg-emerald-500", warn: "bg-amber-500", bad: "bg-red-500" } as const;

export function AppSidebar({ repairs, roles, health = [], engineCount = 0, updateCount = 0, alertSeverity = null, engineTooltip = "Engines", updateTooltip = "Updates", alertTooltip = "Alerts", hardware, budget, runState, ...props }: React.ComponentProps<typeof Sidebar> & { repairs: RepairRecord[]; roles: RoleRecord[]; health?: HealthItem[]; engineCount?: number; updateCount?: number; alertSeverity?: "critical" | "error" | "warning" | null; engineTooltip?: string; updateTooltip?: string; alertTooltip?: string; hardware?: HardwareInfo; budget?: BudgetResponse; runState?: string }) {
  ...
  const common = ["Overview", "Engines", "Models", "Clients", "Tester", "Monitoring"].map((title) => ({ title, url: { Overview: "/", Engines: "/engines", Models: "/models", Clients: "/access", Tester: "/try", Monitoring: "/monitoring" }[title]!, icon: getIcon(iconMap[title as keyof typeof iconMap]), isActive: location.pathname === ({ Overview: "/", Engines: "/engines", Models: "/models", Clients: "/access", Tester: "/try", Monitoring: "/monitoring" }[title]!), badge: title === "Engines" ? engineCount : 0, tooltip: title === "Engines" ? engineTooltip : undefined }));

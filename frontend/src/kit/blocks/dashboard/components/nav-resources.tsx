...
export function NavResources({ capBytes, totalMemoryBytes, freeMemoryBytes, freeDiskBytes, drives = [], pressure, runState }: { capBytes?: number; totalMemoryBytes?: number; freeMemoryBytes?: number; freeDiskBytes?: number; drives?: LiveDrive[]; pressure?: string; runState?: string }) {
  ...
  if (state === "collapsed") return <div className="flex justify-center p-3" title={`Memory ${Math.round(percent)}% used`}><span className="relative flex size-8 items-center justify-center rounded-full border-4 border-muted text-[9px]" aria-label={`Memory ${Math.round(percent)}% used`}>{Math.round(percent)}%</span></div>;

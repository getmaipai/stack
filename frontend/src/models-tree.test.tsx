...
test("Models renders group rollups, routes group actions, renames, and adopts detected stores", async () => {
  ...
  fireEvent.click(document.querySelector('input[value="Family chat"]')!.closest("tr")!);
  ...
  const input = document.querySelector('input[value="Family chat"]')!;
  fireEvent.change(input, { target: { value: "Household" } }); fireEvent.blur(input);
  await waitFor(() => expect(calls.some((call) => call.method === "PATCH" && call.path.includes("/groups/group-family"))).toBe(true));

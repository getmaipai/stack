...
function FactsColumn({ className = "", desktop = false, hardware, healthz, budget, updates, counts, onSpeedTest, speedTesting }: { className?: string; desktop?: boolean; hardware?: HardwareResponse["hardware"]; healthz?: { version: string; uptimeSeconds: number }; budget?: BudgetResponse; updates?: UpdatesResponse; counts: { engines: number; models: number; clients: number }; onSpeedTest: () => void; onCheckStack: () => void; speedTesting: boolean }) {
  ...
  const checkSentence = checking ? "Checking your Stack..." : shownCheck ? shownCheck.reason ?? `Checked ${relativeTime(shownCheck.at).toLowerCase()}, ${shownCheck.ok ? "all good" : "needs attention"}.` : "Not checked yet.";
...
function PhoneOverview({ statusLinks, activityItems, facts }: { statusLinks: Array<{ label: string; value: number; href: string; tone: "default" | "secondary" | "destructive" | "outline" }>; activityItems: Array<{ id: string; primary: string; meta: string }>; facts: ReactNode }) {
  return <main className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-5 px-4 py-5" data-layout="phone"><div className="grid gap-3">{statusLinks.map((status) => <Link to={status.href} key={status.label}><Card><CardContent className="flex items-center justify-between gap-3 p-4"><div><p className="text-sm text-muted-foreground">{status.label}</p><p className="mt-1 text-2xl font-semibold tabular-nums">{status.value}</p></div><Badge variant={status.tone}>{status.label === "Attention" && status.value === 0 ? "All clear" : status.label === "Roles" ? "Serving locally" : "Available now"}</Badge></CardContent></Card></Link>)}</div><FeedCard title="Recent activity" items={activityItems} empty="No notifications yet." more="Show more" /><div className="space-y-5"><Widget title="Usage" description="Requests and tokens served locally."><div className="h-40 rounded-xl bg-muted/50" /></Widget><Widget title="Memory" description="Free memory and pressure."><div className="h-40 rounded-xl bg-muted/50" /></Widget><Widget title="Speed" description="Latest local chat throughput."><p className="text-sm text-muted-foreground">See the full speed chart on a wider screen.</p></Widget></div><details className="rounded-2xl border p-4"><summary className="cursor-pointer font-medium">Computer facts</summary><div className="mt-4">{facts}</div></details></main>;
}
...
export function OverviewPage() {
  ...
  if (layout === "phone") return <><Toaster /><PhoneOverview statusLinks={statusLinks} activityItems={activityItems} facts={<FactsColumn hardware={hardware.data?.hardware} healthz={healthz.data} budget={budget.data} updates={updates.data} counts={{ engines: engines.data?.engines?.length ?? 0, models: models.data?.models?.length ?? 0, clients: clients.data?.clients?.length ?? 0 }} speedTesting={speedTesting} onSpeedTest={() => void runSpeedTestNow()} onCheckStack={() => toast("Coming with the Check my Stack item.")} />} /></>;

...
export function LoginPage({ state }: { state: OperatorState }) {
    ...
    <main className="flex min-h-screen items-center bg-background px-4 py-8 text-foreground sm:px-8">
      <Card className="mx-auto w-full max-w-md">
        <CardHeader>
          ...
          <CardTitle className="text-3xl">{state.state === "setupRequired" ? "Set up access on this computer" : "Welcome back"}</CardTitle>
          <CardDescription className="text-base">{state.state === "setupRequired" ? "Set the operator password on the computer that runs the Stack first." : "Sign in to manage this Stack from another device."}</CardDescription>
        </CardHeader>
        <CardContent>
          {state.state === "setupRequired" ? null : <form className="space-y-6" onSubmit={submit}>

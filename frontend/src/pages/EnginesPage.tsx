...
export function EnginesPage({ Frame }: { Frame: SectionFrameComponent }) {
  ...
  const filterAccessors = useMemo(() => ({
    status: (row: EngineRow) => row.kind === "detected" ? row.store.state === "offline" ? "Offline" : "Detected" : engineStatus(row.engine) === "attention" ? "Needs attention" : engineStatus(row.engine) === "offline" ? "Offline" : "Ready",

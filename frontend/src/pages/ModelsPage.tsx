...
import { ThingsTable, type ThingsTableGroup, type ThingStatus } from "@/kit/blocks/things-table/ThingsTable";
...
type Model = { id: string; roles: string[]; state: string; runtimeState?: string; sizeBytes: number | null; fileMissing?: boolean; measuredFootprintBytes: number | null; estimated: boolean; source: string; licenceSentence?: string; licenceUrl?: string | null; groupId?: string | null; nickname?: string | null; provenance: Record<string, unknown>; modelPath?: string | null };
...
export function ModelsPage({ Frame }: { Frame: SectionFrameComponent }) {
  ...
  if (noModels) return <Frame title="Models" description="Grouped models, measured footprints, nicknames, and utilization."><ScanStatus scan={scanResult} /><PageEmptyState title="No models are installed yet" detail="Add a model or scan this computer to bring the first local model here." action={<div className="flex flex-wrap justify-center gap-3"><Button onClick={() => setAddOpen(true)}>Add model</Button><Button variant="outline" onClick={() => void scanNow()} disabled={scanBusy}>{scanBusy ? "Scanning..." : "Scan this computer"}</Button></div>} /><AddSheet kind="model" open={addOpen} onOpenChange={setAddOpen} onAdded={() => void models.refetch()} /></Frame>;
  return <Frame title="Models" description="Grouped models, measured footprints, nicknames, and utilization."><div className={phone ? "hidden" : "flex justify-end"}><Button onClick={() => setAddOpen(true)}><span aria-hidden="true">＋</span>Add</Button></div><ScanStatus scan={scanResult} /><ThingsPage filter={{ search: { value: filterSearch, onChange: setFilterSearch, placeholder: "Search models" }, groups: filterGroups, onClear: () => { setFilterSearch(""); setFilterSelections({}); } }} table={<ThingsTable<ModelRow>
    columns={[
      ...
      { key: "roles", header: "Roles", width: "17%", render: (row) => row.kind === "detected" ? (row.store.couldHold ?? row.store.roles ?? []).map(roleLabel).join(", ") || "Unassigned" : row.model.roles.map(roleLabel).join(", ") },
      { key: "size", header: "Storage", width: "21%", align: "right", render: (row) => row.kind === "detected" ? `${row.store.candidateModels ?? 0} candidates` : <><div title={row.model.modelPath ?? undefined}>{row.model.sizeBytes ? `${formatBytes(row.model.sizeBytes)} on disk` : "unknown"}</div><div className="text-xs text-muted-foreground">{row.model.fileMissing ? "file missing" : row.model.measuredFootprintBytes ? `${formatBytes(row.model.measuredFootprintBytes)} in memory (measured)` : "Not measured"}</div></> },

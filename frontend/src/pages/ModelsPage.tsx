...
type Model = { id: string; roles: string[]; state: string; runtimeState?: string; sizeBytes: number | null; measuredFootprintBytes: number | null; estimated: boolean; source: string; licenceSentence?: string; licenceUrl?: string | null; groupId?: string | null; nickname?: string | null; provenance: Record<string, unknown> };
...
export function ModelsPage({ Frame }: { Frame: SectionFrameComponent }) {
  ...
  return <Frame title="Models" description="Grouped models, measured footprints, nicknames, and utilization."><div className={phone ? "hidden" : "flex justify-end"}><Button onClick={() => setAddOpen(true)}><span aria-hidden="true">＋</span>Add</Button></div><ScanStatus scan={scanResult} /><ThingsPage filter={{ search: { value: filterSearch, onChange: setFilterSearch, placeholder: "Search models" }, groups: filterGroups, onClear: () => { setFilterSearch(""); setFilterSelections({}); } }} table={<ThingsTable<ModelRow>
    columns={[
      ...
      { key: "size", header: "Storage", width: "21%", align: "right", render: (row) => row.kind === "detected" ? `${row.store.candidateModels ?? 0} candidates` : `${row.model.sizeBytes ? `${Math.round(row.model.sizeBytes / 1_000_000 * 10) / 10} MB` : "Size unknown"} · ${row.model.measuredFootprintBytes ? `${Math.round(row.model.measuredFootprintBytes / 1_000_000 * 10) / 10} MB measured` : "Not measured"}` },

...
export function ModelsPage({ Frame }: { Frame: SectionFrameComponent }) {
  ...
  return <Frame title="Models" description="Grouped models, measured footprints, nicknames, and utilization."><div className="flex justify-end"><Button onClick={() => setAddOpen(true)}><span aria-hidden="true">＋</span>Add</Button></div><ThingsPage filter={{ search: { value: filterSearch, onChange: setFilterSearch, placeholder: "Search models" }, groups: filterGroups, onClear: () => { setFilterSearch(""); setFilterSelections({}); } }} table={<ThingsTable<ModelRow>
    columns={[
      { key: "name", header: "Name", width: "34%", render: (row) => row.kind === "detected" ? <div><p className="font-medium">Detected, not adopted · {row.store.name}</p><p className="text-xs text-muted-foreground">{row.store.version} · {row.store.path ?? row.store.where}</p></div> : <div className="min-w-0"><input aria-label={`Nickname ${row.model.id}`} className="block w-44 max-w-full truncate bg-transparent font-medium outline-none" defaultValue={renames[row.model.id] ?? row.model.nickname ?? ""} placeholder={displayName(row.model.id)} onKeyDown={(event) => { if (event.key === "Escape") { event.currentTarget.value = row.model.nickname ?? ""; event.currentTarget.blur(); } if (event.key === "Enter") event.currentTarget.blur(); }} onBlur={(event) => void renameModel(row.model, event.target.value)} onClick={(event) => event.stopPropagation()} /><p className="truncate text-xs text-muted-foreground">{row.model.nickname && row.model.nickname !== displayName(row.model.id) ? row.model.id : displayName(row.model.id) !== row.model.id ? row.model.id : null}</p><p className="truncate text-xs text-muted-foreground xl:hidden">{groupRows.find((group) => group.id === row.model.groupId)?.name ?? "Ungrouped"}</p></div> },
      { key: "roles", header: "Roles", width: "18%", render: (row) => row.kind === "detected" ? linkCell("/abilities", (row.store.couldHold ?? row.store.roles ?? []).map(roleLabel).join(", ") || "Unassigned") : linkCell("/abilities", row.model.roles.map(roleLabel).join(", ")) },
      { key: "size", header: "Storage", width: "22%", align: "right", render: (row) => row.kind === "detected" ? `${row.store.candidateModels ?? 0} candidates` : `${row.model.sizeBytes ? `${Math.round(row.model.sizeBytes / 1_000_000 * 10) / 10} MB` : "Size unknown"} · ${row.model.measuredFootprintBytes ? `${Math.round(row.model.measuredFootprintBytes / 1_000_000 * 10) / 10} MB measured` : "Not measured"}` },
      { key: "group", header: "Group", width: "14%", compact: true, render: (row) => row.kind === "detected" ? <Badge variant="outline">Adopt</Badge> : <Badge variant="outline" className="max-w-full truncate">{groupRows.find((group) => group.id === row.model.groupId)?.name ?? "Ungrouped"}</Badge> },
      { key: "state", header: "State", width: "12%", align: "right", render: (row) => row.kind === "detected" ? "Not adopted" : <><Badge variant="secondary">{stateLabel(row.model.runtimeState ?? row.model.state)}</Badge>{row.model.estimated && <span className="ml-2 text-xs text-muted-foreground">estimated</span>}</> },
    ]}
    rows={rows}
    groups={groupTree}

...
function ModelsPage() {
  ...
  const [selected, setSelected] = useState<number | null>(null); const [detectedOpen, setDetectedOpen] = useState(false);
  const selectedRole = selected === null ? null : installed[selected] ?? null;
  const model = selectedRole ? modelPanel({ id: selectedRole.model?.id ?? selectedRole.id, nickname: selectedRole.id, group: "Family chat", source: "Local catalog", licence: "Apache-2.0", measuredFootprintBytes: selectedRole.model?.measuredFootprintBytes }, () => {}) : null;
  const detected = detectedPanel("Ollama 0.34 · 127.0.0.1:11434", () => setDetectedOpen(false));
  return <SectionFrame title="Models" description="Installed models and the abilities they serve.">{roles.loading && <Card><CardContent className="p-6"><LoaderCircle className="animate-spin" /></CardContent></Card>}{!roles.loading && installed.length === 0 && <EmptyState title="No models are installed yet" detail="Choose an ability to bring the first local model to this computer." action={<Button asChild><Link to="/abilities">Add abilities</Link></Button>} />}{installed.length > 0 && <div className="space-y-5"><div className={selectedRole ? "pr-0 lg:pr-[29rem]" : ""}><Table>{installed.map((role, index) => <tr className={`cursor-pointer border-b last:border-0 ${selected === index ? "bg-muted/60" : ""}`} key={role.id} onClick={() => setSelected(index)}><td className="p-4 font-medium">{role.model?.id ?? role.id}</td><td className="p-4 text-muted-foreground">{role.description}</td><td className="p-4 text-right"><Badge variant="secondary">{role.state}</Badge></td></tr>)}</Table>{selectedRole && model && <PropertyPanel kind="Model" item={{ id: selectedRole.model?.id ?? selectedRole.id, name: selectedRole.id }} status={selectedRole.state} actions={model.actions} tabs={{ overview: model.overview, insights: model.insights }} open onClose={() => setSelected(null)} />}</div><Card><CardHeader><CardTitle>Detected stores</CardTitle><CardDescription>Local model folders are inspected without changing them.</CardDescription></CardHeader><CardContent><Button variant="outline" onClick={() => setDetectedOpen(true)}>Ollama 0.34 · 3 models</Button></CardContent></Card>{detectedOpen && <PropertyPanel kind="Detected" item={{ name: "Ollama 0.34" }} status="Detected" actions={detected.actions} tabs={{ overview: detected.overview }} open onClose={() => setDetectedOpen(false)} />}</div>}</SectionFrame>;
}

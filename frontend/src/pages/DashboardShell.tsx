...
export function DashboardShell() {
  ...
  return <SidebarProvider><AppSidebar /><SidebarInset><SiteHeader title={title} onSearch={() => setPaletteOpen(true)} /><Routes><Route path="/" element={<BoardPageProxy />} /><Route path="/abilities" element={<AbilitiesProxy />} /><Route path="/models" element={<ModelsPage />} /><Route path="/engines" element={<EnginesPage />} /><Route path="/monitoring" element={<MonitoringPage />} /><Route path="/alerts" element={<AlertsPage />} /><Route path="/updates" element={<SimplePage title="Updates" description="Keep the Stack current without losing control." emptyTitle="Updates are checked on request" emptyDetail="There is nothing to apply yet. The update service will appear here when an update is available." />} /><Route path="/backups" element={<SimplePage title="Backups" description="A quiet place for the data that belongs to your household." emptyTitle="No backup target yet" emptyDetail="Choose where the Stack should keep an encrypted backup before the first backup runs." />} /><Route path="/access" element={<AccessPage />} /><Route path="/try" element={<SimplePage title="Try it" description="A small local place to check that an ability feels right." emptyTitle="Try it opens when your first ability lands" emptyDetail="Choose an ability on the board, then come back here to send a local test." action={<Button asChild><Link to="/abilities">Add abilities</Link></Button>} />} /><Route path="/settings" element={<SimplePage title="Settings" description="The essentials for this local Stack." emptyTitle="Settings stay intentionally small" emptyDetail="Port, data directory, and service state will appear here as the Stack grows." />} /><Route path="*" element={<BoardPageProxy />} /></Routes></SidebarInset><CommandPalette open={paletteOpen} onOpenChange={setPaletteOpen} /></SidebarProvider>;
}

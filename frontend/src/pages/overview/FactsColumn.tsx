...
export function FactsColumn({ hardware, healthz, budget, updates, counts, onSpeedTest, speedTesting }: FactsProps) {
  ...
  const latest = useApiResource<CheckResult | null>("/stack/v1/check/latest");
  async function check(): Promise<void> { setChecking(true); try { const result = await api.post<CheckResult>("/stack/v1/check"); setCheckResult(result); await latest.refetch(); } catch (error) { toast.error(error instanceof Error ? error.message : "The Stack check failed."); } finally { setChecking(false); } }
  const shown = checkResult ?? (latest.data && Array.isArray(latest.data.results) ? latest.data : undefined);
  const sentence = checking ? "Checking your Stack..." : shown ? `Checked ${Math.max(0, Math.round((Date.now() - new Date(shown.at).getTime()) / 60_000))}m ago, ${shown.ok ? "all good" : "needs attention"}.` : "Not checked yet.";
  const facts = [["Uptime", healthz ? uptime(healthz.uptimeSeconds) : "Loading"], ["Memory budget", budget ? bytes(budget.capBytes) : "Loading"], ["Free disk", hardware ? `${bytes(hardware.freeDiskBytes)} free` : "Loading"], ["Operating system", hardware ? osPlain(hardware.osVersion) : "Loading"]];

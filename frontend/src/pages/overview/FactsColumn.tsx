...
export function FactsColumn({ hardware, healthz, budget, updates, counts, onSpeedTest, speedTesting }: FactsProps) {
  ...
  const sentence = checking ? "Checking your Stack..." : shown ? `Checked ${Math.max(0, Math.round((Date.now() - Date.parse(shown.at)) / 60_000))}m ago, ${shown.ok ? "all good" : "needs attention"}.` : "Not checked yet.";

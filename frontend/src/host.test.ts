...
test("desktop tray model maps health and role lines without Tauri", () => {
  ...
  expect(trayMenuModel("paused", [{ label: "Chat", state: "paused" }])).toEqual(["Paused", "Resume", "Open", "—", "Quit"]);
  expect(trayMenu({ daemon: "running", instance: "marlow", state: "running", roles: [{ label: "Chat", state: "ready" }], memory: "12.3 of 24 GB", lastCheck: "Checked 2h ago, all good" })).toEqual(["Running · Chat ready", "Pause", "Open", "—", "Quit"]);
  expect(trayMenu({ daemon: "down", instance: "marlow", state: "running", roles: [], memory: "", lastCheck: "" })).toEqual(["Not running", "Open", "—", "Quit"]);
});

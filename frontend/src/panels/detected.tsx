...
export function detectedPanel(name: string, onAction: (action: string) => void, couldHold?: string[]): DetectedPanelResult {
  ...
  const declared = actionsFor("detected", {}, (action) => onAction(action === "adopt" ? (couldHold ? `adopt:${chosen.join(",")}` : "adopt") : action));
  const adopt = declared[0]!; const forget = declared[1]!;
